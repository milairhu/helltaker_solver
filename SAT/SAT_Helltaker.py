...
fluents = {
    "perso": (2, 2),
    "soldat": [(4, 4), (3, 4), (6, 5), (6, 1), (6, 4), (2, 3), (6, 3)],
    "piegePasFixeActive": [(1, 1), (2, 1)],
    "piegePasFixeAttente": [],
    "porte": (5, 5),
    "block": [(1, 2), (2, 1)],
    "cle": (7, 4),
...
nb_vars = (
    infosGrille["max_steps"] * 10 * infosGrille["n"] * infosGrille["m"]
    + infosGrille["max_steps"] * 4
)  # chrono * nb variable par case * largeur * hauteur + chrono*mouvementpossibles
listeClauses = []
...
def write_dimacs_file(
    dimacs: str, filename: str
):  # écrit dans le DIMACS : write_dimacs_file(dimacsex1,"sudoku1.cnf")
...
def cell_to_variable(
    ...
    largeur: int,
    hauteur: int,
) -> int:  # verifier, mais me parait corrrect
...
def variable_to_cell(
    var: int, largeur: int, hauteur: int, nbCoupsInit: int
) -> Tuple[int, int, int, int]:  # (i,j,CodeCase,NbCoups)
    v = 4 * nbCoupsInit
...
def create_regles_constantes(
    largeur: int, hauteur: int, nbCoupsInit: int
) -> List[List[int]]:
    liste = []
            ...
            for nb in range(1, nbCoupsInit + 1):  # attention au cas 0
                for type in [0, 1, 6]:
                    ...
                    clause.append(
                        ...
                        * (
                            cell_to_variable(
                                i, j, type, nb, nbCoupsInit, largeur, hauteur
                            )
                        )
                    )
                    clause.append(
                        (
                            cell_to_variable(
                                i, j, type, nb - 1, nbCoupsInit, largeur, hauteur
                            )
                        )
                    )
                    liste.append(clause)
    ...
    for i in range(0, hauteur):
        for j in range(0, largeur):
            for nb in range(1, nbCoupsInit + 1):  # attention au cas 0
                clause = []
                clause.append(
                    -1 * (cell_to_variable(i, j, 5, nb, nbCoupsInit, largeur, hauteur))
                )
                clause.append(
                    -1 * (cell_to_variable(i, j, 8, nb, nbCoupsInit, largeur, hauteur))
                )
                clause.append(
                    (cell_to_variable(i, j, 5, nb - 1, nbCoupsInit, largeur, hauteur))
                )
                liste.append(clause)
...
def unique_action(nbActionsInit: int) -> List[List[int]]:
    ...
    for nb in range(1, nbActionsInit + 1):
        vars = range(
            1 + (nb - 1) * 4, 1 + nb * 4
        )  # ensemble de nos variables actions pour le coup nb
...
def create_fait_bonneFin() -> List[
    List[int]
]:  # ATTENTION : cette création dépend de la grille de départ
    global infosGrille
    ...
    for i in range(0, hauteur):
        for j in range(0, largeur):
            if infosGrille["grid"] == "D":
                ...
                for v in voisins:
                    if (
                        v[0] >= 0 and v[0] < hauteur and v[1] >= 0 and v[1] < largeur
                    ):  # si voisin est dans grille

                        if (
                            infosGrille["grid"][v[0]][v[1]] != "#"
                            and infosGrille["grid"][v[0]][v[1]] != "D"
                        ):  # si ce n'est pas un mur ou une autre demonne (il est possible d'aller dessus:
    ...
    for coup in range(0, nbCoupsInit + 1):
        for pos in posSorties:
            clauseDisjonctionPosPerso.append(
                cell_to_variable(pos[0], pos[1], 2, coup, nbCoupsInit, largeur, hauteur)
            )
...
def regles_mouvD(largeur: int, hauteur: int, nbCoupsInit: int) -> List[List[int]]:
    ...
    for i in range(0, hauteur):  # car premiere et derniere lignes sont que des murs
        for j in range(
            0, largeur
        ):  # car premiere et derniere colonnes sont que des murs
            for nb in range(1, nbCoupsInit + 1):
                clause = []
                clause.append(
                    -1 * action_to_variable(nb, "D")
                )  # si on fait l'action d'aller à droite au coup nb
                clause.append(
                    -1 * (cell_to_variable(i, j, 2, nb, nbCoupsInit, largeur, hauteur))
                )  # et perso en i,j
                clause.append(
                    (cell_to_variable(i, j + 1, 4, nb, nbCoupsInit, largeur, hauteur))
                )  # et non block en i+j
                clause.append(
                    (cell_to_variable(i, j + 1, 5, nb, nbCoupsInit, largeur, hauteur))
                )  # et non soldat en i+j
                clause.append(
                    (cell_to_variable(i, j + 1, 1, nb, nbCoupsInit, largeur, hauteur))
                )  # et non mur en i+j
                clause.append(
                    (cell_to_variable(i, j + 1, 3, nb, nbCoupsInit, largeur, hauteur))
                )  # et non porte en i+j
                clause.append(
                    (cell_to_variable(i, j + 1, 6, nb, nbCoupsInit, largeur, hauteur))
                )  # et non piege actif fixe en i+j
                clause.append(
                    (cell_to_variable(i, j + 1, 8, nb, nbCoupsInit, largeur, hauteur))
                )  # et non piege en attente en i+j

                # IMPLIQUE

                clauseNvPosPerso = clause.copy()
                clauseNvPosPerso.append(
                    cell_to_variable(i, j + 1, 2, nb - 1, nbCoupsInit, largeur, hauteur)
                )  # perso en i,j+1
                liste.append(clauseNvPosPerso)  # Clause qui donne la position du perso

                for ip in range(
                    0, hauteur
                ):  # car premiere et derniere lignes sont que des murs
                    for jp in range(
                        0, largeur
                    ):  # car premiere et derniere colonnes sont que des murs
                        if not (ip == i and jp == j + 1):
                            clauseInterditPosPerso = clause.copy()
                            clauseInterditPosPerso.append(
                                -1
                                * (
                                    cell_to_variable(
                                        ip, jp, 2, nb - 1, nbCoupsInit, largeur, hauteur
                                    )
                                )
                            )  # non perso en i,j , et partout ailleurs
                ...
                for type in [3, 4, 5, 9, 10]:
                    for ip in range(0, hauteur):
                        for jp in range(0, largeur):
                            ...
                            if (
                                type == 5
                            ):  # cas du soldat, qui n'a de continuité que si pas sur piege Actif et pas sur Piege en attente
                                ...
                                clauseSpecialise.append(
                                    (
                                        cell_to_variable(
                                            ip, jp, 6, nb, nbCoupsInit, largeur, hauteur
                                        )
                                    )
                                )  # si pas de piege fixe
                                clauseSpecialise.append(
                                    (
                                        cell_to_variable(
                                            ip, jp, 8, nb, nbCoupsInit, largeur, hauteur
                                        )
                                    )
                                )  # et pas de piège en attente
                                clauseSpecialise.append(
                                    (
                                        cell_to_variable(
                                            ip,
                                ...
                                )  # nouvelle situation sans squelette
                                ...
                                for sens in [
                                    1,
                                    -1,
                                ]:  # car continuité est une équivalence. ...=>(nbPortei,j<=>(nb-1)Portei,j)
                                    ...
                                    if (
                                        type == 7
                                    ):  # cas  des pieges en attente qui deviennent actifs
                                        clauseSpecialise.append(
                                            1
                                            * sens
                                            * (
                                        ...
                                        )  # devient un piege actif

                                    elif (
                                        type == 8
                                    ):  # cas  des pieges actifs qui deviennent en attente
                                        clauseSpecialise.append(
                                            1
                                            * sens
                                            * (
        ...
        for j in range(
            ...
            for nb in range(1, nbCoupsInit + 1):
                        ...
                        for jp in range(0, largeur):
                            ...
                            if (
                                ...
                                for sens in [-1, 1]:
                                    ...
                                    if (
                                        type == 7
                                    ):  # cas  des pieges en attente qui passent actifs
                                        clauseSpecialise.append(
                                            1
                                            * sens
                                            * (
                ...
                for type in [
                    ...
                    9,
                    10,
                ]:  # les pieges pas fixes restent inchangés lors de cette opération
                    for ip in range(0, hauteur):
                        for jp in range(0, largeur):
                            ...
                            if (
                                ...
                                for sens in [-1, 1]:
                                    ...
                                    )  # nouvelle situation, on a perdu un coup
    ...
    for i in range(0, hauteur):  # car premiere et derniere lignes sont que des murs
        for j in range(
            ...
            for nb in range(1, nbCoupsInit + 1):
                ...
                clause.append(
                    (cell_to_variable(i, j + 1, 4, nb, nbCoupsInit, largeur, hauteur))
                )  # et non block en i,j+1
                clause.append(
                    (cell_to_variable(i, j + 1, 5, nb, nbCoupsInit, largeur, hauteur))
                )  # et non soldat en i,j+1
                clause.append(
                    (cell_to_variable(i, j + 1, 1, nb, nbCoupsInit, largeur, hauteur))
                )  # et non mur en i,j+1
                clause.append(
                    (cell_to_variable(i, j + 1, 3, nb, nbCoupsInit, largeur, hauteur))
                )  # et non porte en i,j+1
                clause.append(
                    (cell_to_variable(i, j + 1, 6, nb, nbCoupsInit, largeur, hauteur))
                )  # et non piege actif fixe en i,j+1
                clause.append(
                    -1
                    * (cell_to_variable(i, j + 1, 8, nb, nbCoupsInit, largeur, hauteur))
                )  # et piege en attente en i,j+1
    ...
    for i in range(0, hauteur):  # car premiere et derniere lignes sont que des murs
        for j in range(
            ...
            for nb in range(1, nbCoupsInit + 1):
                clause = []
                clause.append(
                    -1 * action_to_variable(nb, "G")
                )  # si on fait l'action d'aller à gauche au coup nb
                clause.append(
                    -1 * (cell_to_variable(i, j, 2, nb, nbCoupsInit, largeur, hauteur))
                )  # et perso en i,j
                clause.append(
    ...
    for i in range(0, hauteur):  # car premiere et derniere lignes sont que des murs
        for j in range(
            ...
            for nb in range(1, nbCoupsInit + 1):
                clause = []
                clause.append(
                    -1 * action_to_variable(nb, "H")
                )  # si on fait l'action d'aller en haut au coup nb
                clause.append(
                    -1 * (cell_to_variable(i, j, 2, nb, nbCoupsInit, largeur, hauteur))
                )  # et perso en i,j
                clause.append(
                    (cell_to_variable(i - 1, j, 4, nb, nbCoupsInit, largeur, hauteur))
                )  # et non block en i+j
                clause.append(
                    (cell_to_variable(i - 1, j, 5, nb, nbCoupsInit, largeur, hauteur))
                )  # et non soldat en i+j
                clause.append(
                    (cell_to_variable(i - 1, j, 1, nb, nbCoupsInit, largeur, hauteur))
                )  # et non mur en i+j
                clause.append(
                    (cell_to_variable(i - 1, j, 3, nb, nbCoupsInit, largeur, hauteur))
                )  # et non porte en i+j
                clause.append(
                    (cell_to_variable(i - 1, j, 6, nb, nbCoupsInit, largeur, hauteur))
                )  # et non piege actif fixe en i+j
                clause.append(
                    (cell_to_variable(i - 1, j, 8, nb, nbCoupsInit, largeur, hauteur))
                )  # et non piege en attente en i+j
                ...
                clauseNvPosPerso = clause.copy()
                clauseNvPosPerso.append(
                    cell_to_variable(i - 1, j, 2, nb - 1, nbCoupsInit, largeur, hauteur)
                )  # perso en i,j+1
                liste.append(clauseNvPosPerso)  # Clause qui donne la position du perso
                    ...
                    for ip in range(0, hauteur):
                        for jp in range(0, largeur):
                            ...
                            clauseSpecialise.append(
                                ...
                                * (
                                    cell_to_variable(
                                        ip, jp, type, nb, nbCoupsInit, largeur, hauteur
                                    )
                                )
                            )  # situation précédente
    ...
    for i in range(0, hauteur):  # car premiere et derniere lignes sont que des murs
        for j in range(
            ...
            for nb in range(1, nbCoupsInit + 1):
                clause = []
                clause.append(
                    -1 * action_to_variable(nb, "B")
                )  # si on fait l'action d'aller en bas au coup nb
                clause.append(
                    -1 * (cell_to_variable(i, j, 2, nb, nbCoupsInit, largeur, hauteur))
                )  # perso en i,j
                clause.append(
...
def main():
    ...
    print(2)
    listeClauses += create_regles_constantes(
        infosGrille["n"], infosGrille["m"], infosGrille["max_steps"]
    )
    print(3)
    ...
    print(5)
    listeClauses += regles_mouvH(
        infosGrille["n"], infosGrille["m"], infosGrille["max_steps"]
    )
    print(6)
    listeClauses += regles_mouvD(
        infosGrille["n"], infosGrille["m"], infosGrille["max_steps"]
    )
    print(7)
    listeClauses += regles_mouvB(
        infosGrille["n"], infosGrille["m"], infosGrille["max_steps"]
    )
    print(8)
    listeClauses += regles_mouvG(
        infosGrille["n"], infosGrille["m"], infosGrille["max_steps"]
    )
    print(9)
    ...
    modele = exec_gophersat(
        "helltaker.cnf", "gophersat"
    )  ##ATTENTION mettre Gophersat dans mon dossier
